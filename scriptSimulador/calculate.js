...
document.getElementById("dataForm").addEventListener("submit", function (event) {
    ...
    const arrayNameB = ["Básico", "Ubicación Geográfica", "Antigüedad", "Ay. Material Didáctico", "Jornada Extendida", "∑ Total Ítems en Blanco"];
    let arrayValorB = new Array(arrayCodigoB.length).fill(0);

    const arrayCodigoG = ["193", "603", "625", "629", "632", "📌"];
    const arrayNameG = ["Adicional Remunerativo Docente C/A (en solo un cargo)", "Plus Unificado Remunerativo (en solo un cargo)", "Plus de Refuerzo Remunerativo (en solo un cargo)", "Adicional Remunerativo 2° Cargo (en solo un cargo)", "Complemento Docente Provincial (en dos cargos)", "∑ Total Ítems Grises"];
    let arrayValorG = new Array(arrayCodigoG.length).fill(0);

    const arrayCodigoN = ["171", "3", "📌"];
    const arrayNameN = ["Compensador Docente Provincial (en dos cargos)", "Salario Familiar", "∑ Total Ítems en Negro"];
    ...
    for (let i = 0; i < cantidadClases; i++) {
        ...
        const topesG = [itemsGrises[0].valor, itemsGrises[1].valor, itemsGrises[2].valor, 2 * itemsGrises[3].valor, 2 * itemsGrises[4].valor];
        ...
        const topesN = [2 * itemsNegros[0].valor, cantHijo * itemsNegros[1].valor, cantHijoDisc * itemsNegros[2].valor, cantEsc * itemsNegros[3].valor, cantEscDisc * itemsNegros[4].valor, cantEsc * itemsNegros[5].valor];
    ...
    //    generarTabla(year, month, arrayCodigoB, arrayNameB, arrayValorB);
...
//"Asignación por hijo", "Asignación por hijo discapacitado", "Asignación por escolaridad",
// "Asignación por escolaridad de hijo discapacitado", "Ayuda Escolar (marzo)"
...
function generarTabla(year, month, arrayCodigo, arrayNombre, arrayValor, arrayColor) {
    ...
    // Actualizar tabla sin agregar duplicados
    ...
    // Evitar duplicaciones eliminando acordeón previo si existe
    let acordeonExistente = document.getElementById("acordeonInfo");
    if (acordeonExistente) {
        acordeonExistente.remove();
    }

    // Agregar acordeón debajo de la tabla
    const valoresD = obtenerValores(year, month, "d"); // Obtener valores del grupo "d"
    const cod210SV = valoresD.length > 0 ? valoresD[0].valor : 0; // Evitar error si no hay valores
    ...
    // Insertar acordeón debajo de la tabla
    tablaResultados.insertAdjacentHTML("afterend", textFinalAcordeon);
    // Desplazar la pantalla hasta la tabla
